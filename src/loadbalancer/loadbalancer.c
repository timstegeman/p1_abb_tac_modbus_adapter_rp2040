...
static uint8_t wait_time, grid_current_data_age;
...
void lb_task(uint32_t now) {
  ...
  if (grid_current_data_age < 0xFF && time_diff < (0xFF - grid_current_data_age)) {
    grid_current_data_age += time_diff;
  } else {
    grid_current_data_age = 0xFF;
  }

  if (grid_current_data_age >= config.fallback_limit_wait_time) {
